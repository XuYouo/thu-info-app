...
description: Implement finance APIs in `packages/thu-info-lib`. Use when working on campus card, transaction history, loss reporting, recharge, bank payment, invoices, graduate income, or finance-sensitive parser changes.
---
...
- Use models under `packages/thu-info-lib/src/models/card/**` and `src/models/home/{bank,invoice}.ts` as the normalized output layer.
